...
pub fn run_true(_args: &[String], _ctx: &mut Context, _stdin: &str) -> Result<Output> {
    Ok(Output::success("".into()))
}

pub fn run_false(_args: &[String], _ctx: &mut Context, _stdin: &str) -> Result<Output> {
    Ok(Output::error(1, "".into(), "".into()))
}

pub fn run_test(args: &[String], ctx: &mut Context, _stdin: &str) -> Result<Output> {
    ...
    let result = match args.as_slice() {
        ...
        [flag, operand] if *flag == "-f" => {
            let abs = VfsPath::join(ctx.get_cwd(), operand);
            ctx.state.vfs.exists(&abs) && !ctx.state.vfs.is_dir(&abs)
        }
        [flag, operand] if *flag == "-d" => {
            let abs = VfsPath::join(ctx.get_cwd(), operand);
            ctx.state.vfs.is_dir(&abs)
        }
        [flag, operand] if *flag == "-e" => {
            let abs = VfsPath::join(ctx.get_cwd(), operand);
            ctx.state.vfs.exists(&abs)
        }
